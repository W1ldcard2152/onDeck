...
import { GOOGLE_SYNC_COMPLETE_EVENT } from '@/contexts/GoogleSyncContext'
...
export function useTasks(
  ...
  // Refetch when the Google sync provider reports a pull that changed local
  // task state. Sync execution itself lives in GoogleSyncContext (single
  // owner); this listener restores the "pulled tasks appear without reload"
  // behavior that previously worked because sync and fetch shared this hook.
  // The 100ms duplicate-fetch guard above dedupes the multi-instance fan-out.
  useEffect(() => {
    if (!userId) return
    const handleSyncComplete = () => {
      fetchTasks()
    }
    window.addEventListener(GOOGLE_SYNC_COMPLETE_EVENT, handleSyncComplete)
    return () => {
      window.removeEventListener(GOOGLE_SYNC_COMPLETE_EVENT, handleSyncComplete)
    }
  }, [userId, fetchTasks])
